...
# Define the alogirithms we will typically use.
ALGORITHMS = {
...
# Provide a summary of the residuals, coefficients, and errors for the model.
...
# Perform linear regression using the input text formula, parsed using the patsy package.
...
# Logistic Regression function, using patsy and text formulas.
...
# Perform linear regression using bootstrap methods, with a default of 100 resampling.
...
# Perform logistic regression using bootstrap methods, with 100 default resample.
...
# Formatting to increse legibility. Not really used.
...
# Generate a table of results from our fitted model.
...
# Output results for bootstrapped linear regression.
def describe_bootstrap_lr(fit, sd=2):
    return ResultsWrapper(fit, sd, True, False)

# Output results for bootstrapped logistic regression.
def describe_bootstrap_lgr(fit, sd=2):
    return ResultsWrapper(fit, sd, True, True)

# Returns how confident we are in the probability input.
def strength(pr):
    if 0 <= pr <= 0.33:
        return "weak"
    if 0.33 < pr <= 0.66:
...
# Calculate and print rho and pearson's correlation coefficients.
...
# Calculate adjusted R squared - evaluate the effect of many variables for linear regression.
